    ...
    ws.on('message', (data) => {
        ...
        switch(message.type) 
        {
            ...
            case MessageType.RESPONSE_BLOCKCHAIN: // 누군가 내가 요청한 블록을 보내주었다. (RESPONSE_BLOCK)
                console.log(ws._socket.remoteAddress, ' : ', message.message); // 우리가 보낸 블록들의 정보가 남겨서 온다.
...
const isValidBlockChain = (receiveBlockchain) => {
    ...
    {
        if (isValidBlockChain(receiveBlockchain[i], receiveBlockchain[i - 1]) == false)
            return false;
    }
...
const responseLatestMessage = () => { // 요청을 받았을때 나의 마지막 블럭을 요청한 쪽에 전달을 해주는 함수 JSON혈태로 return
    return ({   

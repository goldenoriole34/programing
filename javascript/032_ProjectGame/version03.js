...
function tileColor() {
  ...
  if(randomNumber < monsterNum) {
    return monsterColor //몬스터 타일
  }
  else if (randomNumber > monsterNum && randomNumber < 70){
    return forestColor //숲 타일
  }
  else {
    return landColor
  }
...
function setGameFiled(){
  ...
  for ( let i = 0; i <= 10; i++ ) {   // 10 x 10 줄
    tiles[i]=[];
    for (let j = 0; j <= 10; j++) {
      tiles[i][j] = new Tile ( i*60 , j*60 , i*60 , j*60 , tileColor(), randomNumber, false )
        if(randomNumber < monsterNum) {
